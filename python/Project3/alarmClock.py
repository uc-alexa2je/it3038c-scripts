...
while True:
    ...
    if validate != "OK. The format is correct.":
        print(validate)
    else:
        print("You have corrctly enter a time")
        time.sleep(2)
        print(f"You alarm is set for {timeOfAlarm}.")
        break
        ...
        if alarm_hour == present_hour:
            if alarm_min == present_min:
                if alarm_sec == present_sec:
                    ...
                    time.sleep(2)
                    print("If you have the capabilites to play a wav file, your alarm will sound in 6 seconds, otherwise it will throw a code.")
                    time.sleep(6)

                    playsound('C:/it3038c-scripts/python/Project3/alarm-deep_groove.wav')
                    break